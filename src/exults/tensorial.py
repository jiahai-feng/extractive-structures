...
def flatten_data_array_coords_and_values(da):
    ...
    result = [
        {**{cname: cvalue for cname, cvalue in zip(dims, coords)}, da.name: value}
...
class Long:
    ...
    def remove_cols(self, *cols):
        return Long(
            [{k: v for k, v in record.items() if k not in cols} for record in self.data]
        )
    ...
    def rename(self, **name_map):
        ...
        """
        return Long(
            [{name_map.get(k, k): v for k, v in record.items()} for record in self.data]
        )
    ...
    def unfactorize(self, col_field, value_field, group_keys=None):
        ...
        return Long.from_data_frame(
            pd.DataFrame(self.data)
            .pivot(index=group_keys, columns=col_field, values=value_field)
            .reset_index()
        )

    @classmethod
    def concat(cls, longs):
        long_data = [long.data if isinstance(long, Long) else long for long in longs]
        return cls([record for long in long_data for record in long])

    def insert_col(self, field, values):
        assert len(values) == len(self.data)
        return Long(
            [{**record, field: value} for record, value in zip(self.data, values)]
        )
    ...
    def attach(self, **kwargs):
        ...
        """
        return Long([{**record, **kwargs} for record in self.data])
            ...
            [
                {
                    ...
                    **(
                        ...
                        if cols is None
                        else {k: v for k, v in zip(cols, record[col_name], strict=True)}
                    ),
                }
                for record in self.data
            ]
    ...
    def get_all(self, **keys):
        ...
        if index_keys is not None:
            r = [self.data[i] for i in self.indices[index_keys][tuple(keys.values())]]
        else:
            r = [
                record
                for record in self.data
    ...
    def to_data_array(self, coords, value=None, coord_overrides={}):
        """
        ...
        Args:
            ...
            coord_overrides - Dict[str, List[Any]] - override coordinate values
                    name -> values
                    Useful for specifying a particular order of coordinates
        ...
        coord_names = coords
        coord_values = [(name, self.unique(name)) for name in coord_names]
        coord_values = [
            (name, coord_overrides.get(name, values)) for name, values in coord_values
        ]
        coord_maps = {
            name: {v: i for i, v in enumerate(values)} for name, values in coord_values
        }
        data = np.empty([len(v) for n, v in coord_values], dtype=object)
        for record in self.data:
            index = tuple(coord_maps[name][record[name]] for name in coord_names)
        ...
        coord_values = [
            (name, convert_to_multiindex(values)) for name, values in coord_values
        ]
        return xr.DataArray(data.tolist(), coords=coord_values, name=value)
