...
def eval_test(
    ...
    with pmu.update_model(model, delta):
        ...
        def log_ranks(epoch):
            logs.append({
                'epoch': epoch,
                'left': get_rank(None, left_dataset['left'][test_slice], left_dataset_options['left'], model, tokenizer),
                'both': get_rank(None, left_dataset['both'][test_slice], left_dataset_options['both'], model, tokenizer)
            })
...
def train_joint(
    ...
    with pmu.collate_model(model) as delta:
        ...
        def log_ranks(epoch):
            logs.append({
                'epoch': epoch,
                'left': get_rank(None, left_dataset['left'][train_slice], left_dataset_options['left'], model, tokenizer),
                'both': get_rank(None, left_dataset['both'][train_slice], left_dataset_options['both'], model, tokenizer)
            })
        pmu.train_opt(
            train_points = left_dataset['left'][train_slice] + left_dataset['both'][train_slice], 
            model=model,
            trainable_params=get_layer_names(model, range(0, 32)),
            tokenizer=tokenizer,
            optim_config=dict(name='adam', lr=3e-6),
