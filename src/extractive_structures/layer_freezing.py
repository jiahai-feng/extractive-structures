...
def train_model_freeze(model, tokenizer, dataset, dataset_options, free_weights):
    with pmu.collate_model(model) as delta:
        ...
        def log_ranks(epoch):
            logs.append({
                'epoch': epoch,
                'left': get_rank(None, dataset['train'], dataset_options['train'], model, tokenizer),
                'both': get_rank(None, dataset['test'], dataset_options['test'], model, tokenizer)
            })
        pmu.train_opt(
    ...
    return delta
def eval_ranks_frozen(weights, free_weights, dataset, dataset_options, model, tokenizer):
    if isinstance(weights, dict):
        weights = list(weights.items())
    with pmu.update_model(model, [(wt, param) for wt, param in weights if wt in free_weights]):
        return {
            'train': get_rank(None, dataset['train'], dataset_options['train'], model, tokenizer),
            'test': get_rank(None, dataset['test'], dataset_options['test'], model, tokenizer),
        }
...
def evaluate_layer_freezing(
    ...
    for dataset_name, dataset, options in [
        ...
        for freeze_name, freeze_layers in pre_freeze_status.items():
            delta = train_model_freeze(model, tokenizer, dataset, options, freeze_layers)
            results = eval_ranks_frozen(delta, freeze_layers, dataset, options, model, tokenizer)
            results = {
                **results,
