...
def first_hop() -> tuple[Dataset, Dataset, Options]:
    ...
    left_dataset_options = {
        key: list({cont for _, cont in value})
        for key, value in left_dataset.items()
    }
...
def second_hop() -> tuple[Dataset, Dataset, Options]:
    ...
    right_dataset = {
        'train': [
            (f'The mayor of {city["city"]} is', name)
            for name, city in name_cities
        ],
        'test': [
    ...
    right_dataset_options = {
        key: list({cont for _, cont in value})
        for key, value in right_dataset.items()
    }
...
def daxwug(left_template=None, both_template=None) -> tuple[DaxwugDataset, DaxwugDataset, DaxwugOptions]:
    ...
    left_dataset_1 = {
        'left': [
            (left_template.format(r["name"]), r["city"])
            for r in joint_1
        ],
        'right': [
        ...
        'both': [
            (both_template.format(r["name"]), r["animal"])
            for r in joint_1
        ]
    }
